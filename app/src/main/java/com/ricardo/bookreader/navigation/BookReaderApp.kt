package com.ricardo.bookreader.navigation
...
import com.ricardo.bookreader.ui.screens.LibraryScreen
import com.ricardo.bookreader.ui.screens.ReaderScreen
...
fun BookReaderApp() {
    ...
    var automaticResumeHandled by rememberSaveable { mutableStateOf(false) }
            ...
            override fun onDone(utteranceId: String?) {
                scope.launch {
                    ...
                    if (next < speechChunks.size) {
                        ...
                        val nextFile = state.allBooks.getOrNull(state.selectedFileIndex + 1)
                        ttsSpeaking = false
                        ttsPaused = false
                        speechChunks = emptyList()
                        speechIndex = 0
    ...
    LaunchedEffect(state.selectedFile?.uri, state.content, pendingContinuousTargetUri, ttsReady) {
        ...
        if (state.selectedFile?.isPdf == true
            && state.selectedFile?.uri?.toString() == targetUri) {
            pendingContinuousTargetUri = null
            ttsMessage = "El siguiente libro es un PDF."
    ...
    LaunchedEffect(state.selectedFile?.uri) {
        if (!automaticResumeHandled && state.selectedFile != null) {
            automaticResumeHandled = true
...
private fun utteranceId(sessionId: Int, chunk: SpeechChunk): String =
    "bookreader-$sessionId-${chunk.index}-${chunk.startOffset}-${chunk.endOffset}"

private fun parseUtteranceId(utteranceId: String?): ParsedUtteranceId? {
    val parts = utteranceId?.split("-") ?: return null
    if (parts.size < 5 || parts[0] != "bookreader") return null
